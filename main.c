...
main(int argc, char **argv)
{
	...
	if (r) {
		printf("Executable did not verify as CGC\n");
		return (1);
	}
...
cgcef_verify_ident(CGCEf *cgcef)
{
	...
	if (ident[EI_MAG0] != CGCEFMAG0 ||
		...
		warnx("did not identify as a CGC binary (ident %c%c%c)",
	...
	if (ident[EI_OSABI] != CGCEFOSABI_CGCOS) {
		warnx("did not identify as a CGC OS ABI binary");
		r = -1;
	}
	if (ident[EI_ABIVERSION] != 1) {
		warnx("did not identify as a v1 CGC OS ABI binary");
		r = -1;
	}
...
cgcef_verify_section_hdrs(CGCEf *cgcef)
{
	...
	if (ehdr->e_shnum) {
		warnx("WARNING: CGC Executable contained optional section headers");
		/* not a fatal error */
	}
